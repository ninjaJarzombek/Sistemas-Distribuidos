...
public class ControlTower extends UnicastRemoteObject implements RemoteInterface {
  ...
  /**
    ...
    @Override
    public Boolean checkInTriciclo(Triciclo a) throws RemoteException {
        if(this.mapaPista[a.getX()][a.getY()] == null)
            this.mapaPista[a.getX()][a.getY()] = a;
        return true;
    }
    @Override
    public Triciclo getTriciclo(String id, int c) throws RemoteException {
        Triciclo a = null;
        for (int i = 0; i < 8; i++) {
            if(this.mapaPista[c][i] != null){
                a = (Triciclo) this.mapaPista[c][i];
                if(!a.getId().equals(id))
                    a = null;
            }
        }
    ...
    @Override
    public Boolean moverTriciclo(Triciclo a, int c) throws RemoteException {
        Boolean flag = false;
        int newY = a.getY() + 1;
        if (newY < 8) {
            try {
    ...
    @Override
    public Boolean checkInPatinDelDiablo(PatinDelDiablo a) throws RemoteException {
        if(this.mapaPista[a.getX()][a.getY()] == null)
            this.mapaPista[a.getX()][a.getY()] = a;
        return true;
    }
    @Override
    public PatinDelDiablo getPatinDelDiablo(String id, int c) throws RemoteException {
        PatinDelDiablo a = null;
        for (int i = 0; i < 8; i++) {
            if(this.mapaPista[c][i] != null){
                a = (PatinDelDiablo) this.mapaPista[c][i];
                if(!a.getId().equals(id))
                    a = null;
            }
        }
    ...
    @Override
    public Boolean moverPatinDelDiablo(PatinDelDiablo a, int c) throws RemoteException {
        Boolean flag = false;
        int newY = a.getY() + 1;
        if (newY < 8) {
            try {
